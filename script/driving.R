...
for(i in 1:nrow(query_frame)){
  ...
  if(input_type=='string'){
    ...
    dirs = google_directions(origin = o_xy,
                             ...
                             traffic_model = traffic_model_sel)
  } else {
    oid = query_frame$origin_id[i]
  ...
  if(dirs$status=="OK"){
    duration_no_traffic_min <- round(dirs$routes$legs[[1]]$duration$value/60 %>% round(1), 1)
    duration_with_traffic_min <- round(dirs$routes$legs[[1]]$duration_in_traffic$value/60, 1)
    distance_mi <- round(dirs$routes$legs[[1]]$distance$value*0.000621371,1)
    
    query_frame$duration_min[i] <- duration_no_traffic_min
    query_frame$duration_with_traffic_min[i] <- duration_with_traffic_min 
